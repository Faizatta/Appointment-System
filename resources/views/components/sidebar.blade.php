    ...
    <ul class="nav flex-column mt-1">
        ...
        <li class="nav-item">
            <a class="nav-link {{ request()->is('doctors*') ? 'active' : '' }}"
               href="{{ auth()->user()->hasRole('admin') || auth()->user()->can('view doctors') ? route('doctors.index') : '#' }}"
               style="{{ auth()->user()->hasRole('admin') || auth()->user()->can('view doctors') ? '' : 'pointer-events: none; opacity: 0.5;' }}">
        ...
        <li class="nav-item">
            <a class="nav-link {{ request()->is('patients*') ? 'active' : '' }}"
               href="{{ auth()->user()->hasRole('admin') || auth()->user()->can('view patients') ? route('patients.index') : '#' }}"
               style="{{ auth()->user()->hasRole('admin') || auth()->user()->can('view patients') ? '' : 'pointer-events: none; opacity: 0.5;' }}">
